...
public class Leilao implements Serializable {
    ...
    public void propoe(Lance lance){
        ...
        if (primeiroLance(valorLance)) return;
        Collections.sort(lances);
        calculaMaiorLance(valorLance);
    }

    private boolean primeiroLance(double valorLance) {
        if(lances.size() == 1){
    ...
    private void valida(Lance lance) {
        double valorLance =  lance.getValor();
        if (lanceMenorQueUltimo(valorLance)) throw new LanceMenorQueUltimoLanceException();
        if(temLances()){
            Usuario usuarioNovo = lance.getUsuario();
            if (usuarioForIgualAoUltimo(usuarioNovo)) throw new LanceSeguidoDoMesmoUsuarioException();
            if (usuarioDeuCincoLances(usuarioNovo)) throw new UsuarioDeuCincoLancesException();
        }
    }
    ...
    private boolean usuarioForIgualAoUltimo(Usuario usuarioNovo) {
    ...
    private boolean lanceMenorQueUltimo(double valorLance) {
