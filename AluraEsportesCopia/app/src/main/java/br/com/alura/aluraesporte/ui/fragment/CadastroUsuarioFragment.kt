...
class CadastroUsuarioFragment :Fragment() {
    ...
    private fun cadastra(usuario: Usuario) {
        viewModel.cadastre(usuario).observe(viewLifecycleOwner, Observer {
                it?.let{ recurso ->
                    if(recurso.dado){
                        view?.snackBar(
                            "Cadastro realizado")
                        ...
                        val mensagemErro = recurso.erro ?: "Ocorreu um erro"
    ...
    private fun validaCampos(
        email: String,
        senha: String,
        confirmaSenha:String): Boolean {

        var valido = true
        if(email.isBlank()){
            cadastro_usuario_email.error = "Email é necessário"
            valido = false
        }
        if(senha.isBlank()){
